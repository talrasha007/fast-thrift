...
TFramedTransport.prototype = {
  ...
  flush: function() {
      if (this.onFlush) {
          ...
          binary.writeI32(out, this.outCount);
...
var TBufferedTransport = exports.TBufferedTransport = function(buffer, callback) {
  ...
  this.outBuffers = new Buffer(this.writeBufferSize);
  this.outCount = 0;
  this.onFlush = callback;
};
...
TBufferedTransport.prototype = {
  ...
  write: function(buf, len) {
    ...
    if (this.outCount + len > this.writeBufferSize) {
      this.flush();
      if (len >= this.writeBufferSize) return this.onFlush && this.onFlush(buf);
    }
